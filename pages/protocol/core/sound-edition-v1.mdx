...
[`contracts/core/SoundEditionV1.sol`](https://github.com/soundxyz/sound-protocol/blob/master/contracts/core/SoundEditionV1.sol)

The NFT contract, an implementation of [ERC721A](https://github.com/chiru-labs/ERC721A) deployed via the [ERC-1167 minimal proxy clone](https://eips.ethereum.org/EIPS/eip-1167) pattern.

It has a [`mint`](sound-edition-v1#mint) function which allows authorized minter contracts or administrators to batch mint NFTs (authorization is granted via the [`MINTER_ROLE`](sound-edition-v1#minter_role) or [`ADMIN_ROLE`](sound-edition-v1#admin_role) respectively).

These roles can be granted/revoked by the owner, or during setup by the SoundCreator.
...
- The caller must be the owner of the contract, or have either the
  [`ADMIN_ROLE`](sound-edition-v1#admin_role), [`MINTER_ROLE`](sound-edition-v1#minter_role), which can be granted via `grantRoles`.
...
- The caller must be the owner of the contract, or have the
  [`ADMIN_ROLE`](sound-edition-v1#admin_role), which can be granted via `grantRoles`.
...
- The caller must be the owner of the contract, or have the [`ADMIN_ROLE`](sound-edition-v1#admin_role).
...
Returns the bit flag to freeze the metadata on initialization.
...
Returns the bit flag to enable the mint randomness feature on initialization.
...
Returns a pseudorandom value based on latest block hash, which is stored upon each mint unless [mintConcluded](#mintconcluded) is true. Used for game mechanics like the Sound Golden Egg. Returns 0 before revealed.
